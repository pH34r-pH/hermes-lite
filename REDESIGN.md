...
6. **CICD owns deploy; hermes-lite owns three repos.** Hermes-lite
   ...
   - **`azure-api`** — the infra-as-code + Azure Static Web App
...
| **Ollama (partner, on `azure-api` VM)** | Same adapter, different base URL | Self-hosted partner small model exposed through the developer-owned API (§5.11); rate-limit-free experimentation target for API-driven skill development |
...
6. `arxiv-write` — produce a research note with citations resolved
   ...
   **knowledge repo** (`~/repos/knowledge/notes/<topic>.md`) so it
   ...
   independently of the `azure-api` and `hermes-lite` codebases.
...
workspace alongside `azure-api` and `hermes-lite`, so `arxiv-write`
commits notes
...
**Partner Small Model (deployed on the `azure-api` VM).** A central
...
spec → plan → tasks → PR cycle against `azure-api`'s `infra/`
modules.
...
- **Knowledge repo** (`~/repos/knowledge`): a dedicated Git repo,
  ...
  `azure-api` and the `hermes-lite` source fork; see §1 premise
  ...
  and rebased onto its own `main` independently of `azure-api`
...
| `azure-api` repo (`~/repos/azure-api`: infra/web/api/specs/security) | ~10 GB |
...
(§5.10) is the bridge; CICD is the deploy boundary for `azure-api`;
...
target as the worked example for the `azure-api` repo.
...
budget. CICD scope is intentionally narrow: only `azure-api` has
a deploy pipeline, because only `azure-api` ships running cloud
resources.

**Repo 1 — `azure-api`** (`~/repos/azure-api`, workspace name
`azure-api`, `approval_mode: pr-only`, branch prefix `hermes/`):
...
- `specs/` — spec-kit output for changes that target the agent
  ...
  the diagnostics layer land here, not in `azure-api`.
...
`knowledge` names the **target repo** (`azure-api` or
`hermes-lite`) so `spec-specify` writes the resulting `spec.md`
into the correct workspace. Citations in `azure-api/specs/` and
...
Azure, and it is scoped to `azure-api` alone. The agent's
...
`azure-api`, the OpenAI-compatible adapter retained in §3.1 is
...
1. **Discord (or Open WebUI / TUI) prompt.** The user posts a
   directive in an allowlisted channel — either an `azure-api`
...
3. **Propose.** The orchestrator unloads the arXiv kit and loads
   ...
   **target repo** (`azure-api` or `hermes-lite`); memory points
   at `spec + dev + api` (for `azure-api`) or `spec + dev` (for
...
5. **Plan.** `spec-plan` writes `specs/<feature>/plan.md`. For
   `azure-api` it may span `infra/`, `web/`, and `api/` in one
...
10. **Self-pentest.** Before opening the PR, the orchestrator
    ...
    `azure-api` PRs the scan also includes `sec-config-review`
    ...
    `security/findings/` (in `azure-api`) or surfaced in the PR
...
13. **CICD or rolling pull.**
    - For `azure-api`: the single CICD pipeline deploys `infra/`,
...
14. **Verify.** Post-deploy on `azure-api`: `web-ops` runs a
...
2. **Security kit gates production traffic.** Before the next loop
   ...
   `sec-web-probe` (for `azure-api`) or `sec-static-scan` (for
...
4. **Self-modifying changes are not exempt from review.** A spec
   ...
   sequence as a spec that targets `azure-api`. There is no
...
- **Offline parity.** The loop above is designed to degrade
  ...
  edits all keep working against `~/repos/azure-api`,
...
- Scaffold the **three first-class workspaces** and register them
  in `workspaces.yaml`:
  - `azure-api` (`infra/`, `web/`, `api/`, `specs/`, `security/`,
...
drafts a spec proposal against either `azure-api` or
`hermes-lite`, opens a PR after a passing self-pentest, merges,
CICD deploys (`azure-api`) or rolling-pull installs
...
the `azure-api` and `hermes-lite` codebases, so contributors and
...
| `agent/chat_completion_helpers.py` | Retain | OpenAI-compatible helper; the `azure-api` API and the partner small model are reached through this. |
...
first-class repos** on the cyberdeck — `azure-api` (infra-as-code
...
`azure-api` CICD pipeline deploys infra, web, api, and the partner
...
to `azure-api`" — or "tighten the `arxiv-extract` JSON schema in
