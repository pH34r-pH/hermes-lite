...
# platform is sufficiently configured to be considered "connected".
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    Platform.API_SERVER: lambda cfg: True,
    Platform.OPENWEBUI: lambda cfg: True,
}
...
class GatewayConfig:
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        ...
        # Generic token/api_key auth covers Discord, etc.
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                if plat in {Platform.DISCORD, Platform.OPENWEBUI} and "channel_skill_bindings" in platform_cfg:
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    # Open WebUI
    openwebui_enabled = os.getenv("OPENWEBUI_ENABLED", "").lower() in {"true", "1", "yes"}
    openwebui_url = os.getenv("OPENWEBUI_URL", "")
    openwebui_api_key = os.getenv("OPENWEBUI_API_KEY", "")
    if openwebui_enabled or openwebui_url or openwebui_api_key:
        if Platform.OPENWEBUI not in config.platforms:
            config.platforms[Platform.OPENWEBUI] = PlatformConfig()
        config.platforms[Platform.OPENWEBUI].enabled = True
        if openwebui_url:
            config.platforms[Platform.OPENWEBUI].extra["url"] = openwebui_url
        if openwebui_api_key:
            config.platforms[Platform.OPENWEBUI].extra["api_key"] = openwebui_api_key
        openwebui_model = os.getenv("OPENWEBUI_MODEL", "")
        if openwebui_model:
            config.platforms[Platform.OPENWEBUI].extra["model"] = openwebui_model
        openwebui_timeout = os.getenv("OPENWEBUI_TIMEOUT", "")
        if openwebui_timeout:
            try:
                config.platforms[Platform.OPENWEBUI].extra["timeout"] = int(openwebui_timeout)
            except ValueError:
                pass
