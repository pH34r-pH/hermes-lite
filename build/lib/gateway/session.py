...
# WhatsApp identity helpers removed in hermes-lite
def canonical_whatsapp_identifier(chat_id):
    return chat_id
def normalize_whatsapp_identifier(chat_id):
    return chat_id
...
_PII_SAFE_PLATFORMS: frozenset = frozenset()
"""Platforms where user IDs can be safely redacted.
All previously allowlisted platforms have been removed in hermes-lite.
Discord is excluded because mentions use ``<@user_id>``
...
def build_session_key(
    ...
    if source.chat_type == "dm":
        ...
        # WhatsApp platform removed in hermes-lite
    ...
    # WhatsApp platform removed in hermes-lite — no canonicalization needed
