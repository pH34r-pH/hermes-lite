...
from .profiles import (
    CANONICAL_PROFILES,
    CompositeRecallQuery,
    MemoryWriteDenied,
    ProfileBinding,
    ProviderNamespaceMap,
...
class ProfiledMemoryProvider:
    """Wraps a MemoryProvider to enforce profile namespaces and security guard."""
    ...
    def __init__(
        ...
        self._kit = kit
        self._binding = binding or ProfileBinding.from_config()
        self._composite = CompositeRecallQuery(self._binding)
        self._profiles = self._binding.resolve(kit)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        # Pass profile namespace to provider if it supports it
        if self._profiles:
            kwargs["memory_profiles"] = self._profiles
        self._provider.initialize(session_id, **kwargs)

    def system_prompt_block(self) -> str:
        block = self._provider.system_prompt_block()
        if self._profiles:
            block += f"\n[Active memory profiles: {', '.join(self._profiles)}]\n"
        return block

    def prefetch(self, query: str, *, session_id: str = "") -> str:
        # For composite kits, query all bound profiles and merge
        if len(self._profiles) > 1:
            results_by_profile: Dict[str, List[dict]] = {}
            for profile in self._profiles:
                # Providers that support namespace argument
                try:
                    text = self._provider.prefetch(query, session_id=session_id)
                    results_by_profile[profile] = [{"text": text, "metadata": {}}]
                except Exception:
                    results_by_profile[profile] = []
            merged = self._composite.merge_results(self._kit, results_by_profile)
            texts = [m.get("text", "") for m in merged if m.get("text")]
            return "\n".join(texts)
        return self._provider.prefetch(query, session_id=session_id)

    def sync_turn(self, user: str, asst: str, **kwargs) -> None:
        # Route write to primary profile
        primary = self._profiles[0] if self._profiles else "dev"
        SecurityProfileGuard.check_write(self._kit, primary)
        if "memory_profile" not in kwargs:
            kwargs["memory_profile"] = primary
        self._provider.sync_turn(user, asst, **kwargs)

    def get_tool_schemas(self) -> List[dict]:
        return self._provider.get_tool_schemas()

    def handle_tool_call(self, name: str, args: dict) -> str:
        # If the tool call targets a specific profile, enforce guard
        profile = args.get("profile") or (self._profiles[0] if self._profiles else "dev")
        if args.get("action") in ("write", "store", "save", "update"):
            SecurityProfileGuard.check_write(self._kit, profile)
        return self._provider.handle_tool_call(name, args)

    def shutdown(self) -> None:
        self._provider.shutdown()

    def on_turn_start(self, turn, message, **kwargs) -> None:
        if hasattr(self._provider, "on_turn_start"):
            self._provider.on_turn_start(turn, message, **kwargs)

    def on_session_end(self, messages) -> None:
        if hasattr(self._provider, "on_session_end"):
            self._provider.on_session_end(messages)

    def on_session_switch(self, new_session_id, **kwargs) -> None:
        if hasattr(self._provider, "on_session_switch"):
            self._provider.on_session_switch(new_session_id, **kwargs)

    def on_pre_compress(self, messages) -> str:
        if hasattr(self._provider, "on_pre_compress"):
            return self._provider.on_pre_compress(messages)
        return ""

    def on_memory_write(self, action, target, content, metadata=None) -> None:
        profile = (metadata or {}).get("profile") or (self._profiles[0] if self._profiles else "dev")
        if action in ("write", "store", "save", "update"):
            SecurityProfileGuard.check_write(self._kit, profile)
        if hasattr(self._provider, "on_memory_write"):
            self._provider.on_memory_write(action, target, content, metadata=metadata)

    def on_delegation(self, task, result, **kwargs) -> None:
        if hasattr(self._provider, "on_delegation"):
            self._provider.on_delegation(task, result, **kwargs)
