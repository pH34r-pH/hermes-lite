...
def init_agent(
    ...
    agent.prompt_profile = prompt_profile
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server", "ollama_chat"}:
        ...
        # Native Ollama adapter — bypasses OpenAI-compatible endpoint.
        agent.api_mode = "ollama_chat"
    else:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if not agent.quiet_mode:
                ...
                try:
                    ...
                    pass  # azure_identity_adapter removed in hermes-lite
        ...
        if api_key and base_url:
            ...
            # hermes-lite — custom headers for removed providers (OpenRouter,
            # NVIDIA NIM, RouterMint, Kimi, Qwen) are not needed.
            try:
            ...
            # No explicit creds — hermes-lite only supports explicit
            # credentials or Ollama local (which uses no api_key).
            if agent.provider == "ollama":
                client_kwargs = {"base_url": base_url or "http://127.0.0.1:11434"}
                if _provider_timeout is not None:
                    client_kwargs["timeout"] = _provider_timeout
            else:
                # Fail fast for non-Ollama providers without credentials
                _env_hint = "ANTHROPIC_API_KEY"
                if agent.provider and "openai" in agent.provider.lower():
                    _env_hint = "OPENAI_API_KEY"
                raise RuntimeError(
                    f"No API key configured for provider '{agent.provider}'. "
                    f"Set {_env_hint} in ~/.hermes-lite/.env or use --provider ollama."
                )
