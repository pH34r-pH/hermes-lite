...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    def _call():
        try:
            ...
            elif agent.api_mode == "ollama_chat":
                # Ollama native API — uses httpx directly.
                from agent.ollama_adapter import OllamaAdapter
                adapter = OllamaAdapter(
                    base_url=agent.base_url or "http://127.0.0.1:11434",
                    model=agent.model,
                    context_window=getattr(agent, "_ollama_num_ctx", 32768),
                )
                result["response"] = adapter.chat(**api_kwargs)
            else:
                request_client = _set_request_client(
                    agent._create_request_openai_client(
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # Ollama native API — bypasses OpenAI-compatible endpoint.
    if agent.api_mode == "ollama_chat":
        _ot = agent._get_transport()
        return _ot.build_kwargs(
            model=agent.model,
            messages=api_messages,
            tools=tools_for_api,
    ...
    # Temperature: removed provider-specific fixed-temperature helpers.
    _omit_temp = False
    _fixed_temp = None
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Build a minimal OpenAI client for the fallback provider
        import openai
        fb_client_kwargs = {"api_key": fb_api_key_hint or os.getenv("OPENAI_API_KEY", ""), "base_url": fb_base_url_hint or os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1")}
        fb_client = openai.OpenAI(**fb_client_kwargs)
        _resolved_fb_model = fb_model
    except Exception:
        fb_client = None
        _resolved_fb_model = None
    ...
    try:
        ...
        if _is_nous:
            try:
                ...
                pass  # portal_tags removed in hermes-lite
        if agent.api_mode == "codex_responses":
